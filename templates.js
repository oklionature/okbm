// =========================================================================
// 🚀 [templates.js] 20종 템플릿 엔진 & 스와이프 제스처 시스템
// =========================================================================

// 🚪 1. 공유 모달 열기 & 스와이프 제스처 자동 연결
// 💾 [필수] 배낭 패킹 저장 & 카드 모달 호출 함수 (중복 방지 완벽 처리)
function saveCurrentPackingRecord() {
  const allItems = [];
  if (typeof CATEGORIES !== 'undefined') {
    CATEGORIES.forEach(c => {
      (selectedGearMap[c.id] || []).forEach(it => { if (it.weight > 0) allItems.push(it); });
    });
  }

  if (allItems.length === 0) return showToast('선택된 장비가 없습니다. 배낭에 장비를 담아주세요!', 'warn');

  const totalGrams = allItems.reduce((sum, g) => sum + Number(g.weight), 0);
  const totalKg = (totalGrams / 1000).toFixed(2);
  const currentItemsStr = allItems.map(g => `${g.name} (${g.weight}g)`).sort().join(' / ');
  const nowTime = Date.now();

  // 중복 저장 방지 (동일 세팅이면 새로 만들지 않고 기존 기록 오픈)
  if (typeof safeGetJSON === 'function') {
    packingHistoryList = safeGetJSON('okbm_packing_history', []);
    if (packingHistoryList.length > 0) {
      const lastRecord = packingHistoryList[0];
      const lastItemsStr = (lastRecord.items || []).slice().sort().join(' / ');
  ...
  const newRecord = {
    ...
    items: allItems.map(g => `${g.name} (${g.weight}g)`),
    photo: ''
  };

  if (typeof packingHistoryList !== 'undefined') {
    packingHistoryList.unshift(newRecord);
    if (packingHistoryList.length > 20) packingHistoryList = packingHistoryList.slice(0, 20);
...
function openPackShareModal(record, items, forceStudio = false) {
  currentShareRecord = record || { id: 'pack_' + Date.now(), date: new Date().toLocaleDateString(), weightKg: '0.00', weightGrams: 0, items: [] };
  currentShareItems = items || (currentShareRecord.items ? currentShareRecord.items.map(str => {
    const m = str.match(/^(.*?)\s*\((\d+)g\)$/);
  ...
  const spotInput = document.getElementById('shareCardSpotInput');
  const memoInput = document.getElementById('shareCardMemoInput');
  const clearBtn = document.getElementById('btnSpotInputClear');

  if (spotInput) { 
    spotInput.value = currentShareRecord.spot || '강원 대관령 선자령 (832m)'; 
    if (clearBtn) clearBtn.style.display = spotInput.value ? 'flex' : 'none'; 
  }
  if (memoInput) memoInput.value = currentShareRecord.memo || '비화식으로 즐기는 조용한 하룻밤';

  const editBox = document.querySelector('#packShareModalOverlay .custom-modal > div:nth-child(2)');
  const vaultBtn = document.getElementById('btnSaveToVaultMain');
  if (openedFromBasecamp) { 
  ...
  if (forceStudio && currentSharePhoto) { 
    openPhotoStudio(); 
    return; 
  }

  document.getElementById('packShareModalOverlay').style.display = 'flex';
  if (typeof closePackingModal === 'function') closePackingModal();
  switchShareCardTemplate(selectedTemplateId);

  // ✨ 스와이프 제스처 즉시 활성화
  setTimeout(() => initCardSwipeGesture(), 80);
}

// 🏷️ 2. 템플릿 전환 & 상단 칩 이름 바 자동 스크롤 연동
function switchShareCardTemplate(tmplId, isSwipe = false) {
  let targetId = Number(tmplId);
  ...
  const chips = document.querySelectorAll('.tmpl-chip-btn');
  chips.forEach((btn, idx) => {
    const isActive = (idx + 1) === selectedTemplateId;
...
function updateShareCardLive() {
  const container = document.getElementById('packShareCaptureArea');
  if (!container) return;
  const spotInput = document.getElementById('shareCardSpotInput');
  const memoInput = document.getElementById('shareCardMemoInput');
  const spotVal = (spotInput && spotInput.value.trim()) ? spotInput.value.trim() : '강원 대관령 선자령 (832m)';
  const memoVal = (memoInput && memoInput.value.trim()) ? memoInput.value.trim() : '비화식으로 즐기는 조용한 하룻밤';
...
// 🖐️ 4. [핵심] 카드 좌우 스와이프 제스처 인터랙션 엔진
let cardTouchStartX = 0;
let cardTouchStartY = 0;
let cardTouchStartTime = 0;
let isCardSwiping = false;
let isCardPointerDown = false;

function initCardSwipeGesture() {
  const card = document.getElementById('packShareCaptureArea');
  if (!card) return;
  ...
  function handleMove(clientX, clientY) {
    ...
    const diffX = clientX - cardTouchStartX;
    const diffY = clientY - cardTouchStartY;
    ...
    if (absX > 6 && absX > absY) {
      ...
      card.style.opacity = `${Math.max(0.6, 1 - (absX / 500))}`;
    }
  }
  ...
  card.addEventListener('touchend', (e) => {
    const endX = (e.changedTouches && e.changedTouches[0]) ? e.changedTouches[0].clientX : cardTouchStartX;
    const endY = (e.changedTouches && e.changedTouches[0]) ? e.changedTouches[0].clientY : cardTouchStartY;
    handleEnd(endX, endY);
  }, { passive: true });
...
// 🎨 5. [총 20대 전체 템플릿 마크업 엔진]
function generateCardMarkup(tmplId, record, items, spot, memo) {
  const profile = typeof safeGetJSON === 'function' ? safeGetJSON('user_profile', null) : null;
  const nick = profile ? profile.nickname : '낭만탐험가';
  const weight = record ? record.weightKg : '0.00';
  const dateStr = (record && record.date) ? record.date : (() => {
    const d = new Date(); return `${d.getFullYear()}.${String(d.getMonth()+1).padStart(2,'0')}.${String(d.getDate()).padStart(2,'0')}`;
  })();

  const targetSpot = spot || '강원 대관령 선자령 (832m)';
  const targetMemo = memo || '비화식으로 즐기는 조용한 하룻밤';
  const list = items || [];

  const logoDark = SVG_ICONS.brandLogo('#000000', '#0284c7');
  const logoWhite = SVG_ICONS.brandLogo('#ffffff', '#38bdf8');
  const logoNavy = SVG_ICONS.brandLogo('#1e3a8a', '#b91c1c');
  const logoPink = SVG_ICONS.brandLogo('#f43f5e', '#fde047');
  const logoSunset = SVG_ICONS.brandLogo('#ea580c', '#fb7185');
  const logoSage = SVG_ICONS.brandLogo('#15803d', '#86efac');
  const logoTeal = SVG_ICONS.brandLogo('#0d9488', '#5eead4');

  const baseStyle = `width:100%; aspect-ratio:3/4; max-width:330px; margin:0 auto; box-sizing:border-box; border-radius:14px; box-shadow:0 12px 30px rgba(0,0,0,0.9); display:flex; flex-direction:column; justify-content:space-between; overflow:hidden; position:relative; touch-action:pan-y;`;

  const makePledge = (color, bg, border, sub) => `
    <div style="margin-top:4px; padding:4px 6px; border:1px dashed ${border}; background:${bg}; border-radius:6px; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:1px; text-align:center;">
  ...
  switch (Number(tmplId)) {
    case 1: // 🧾 01. 영수증
      return `
        <div style="${baseStyle} background:#f4f1ea; color:#1c1917; padding:12px 11px; font-family:'JetBrains Mono', monospace; border-top:3px dashed #78716c; border-bottom:3px dashed #78716c;">
    ...
    case 2: // 🎫 02. 보딩패스
      return `
        <div style="${baseStyle} background:#0f172a; border:1px solid #334155; padding:12px 11px; font-family:'Space Grotesk', sans-serif; color:#ffffff;">
          <div style="display:flex; flex-direction:column; gap:4px; flex:1;">
    ...
    case 3: // 📮 03. 에어메일
      return `
        <div style="${baseStyle} background:#fcfbf7; color:#1e293b; padding:12px 10px; font-family:'Noto Serif KR', serif; border:5px solid transparent; border-image: repeating-linear-gradient(135deg, #1e3a8a 0, #1e3a8a 10px, transparent 10px, transparent 15px, #b91c1c 15px, #b91c1c 25px, transparent 25px, transparent 30px) 10;">
          <div style="display:flex; flex-direction:column; gap:4px; flex:1;">
    ...
    case 4: // 🏛️ 04. 뮤지엄
      return `
        <div style="${baseStyle} background:#f4f6f4; color:#1c1917; padding:12px 11px; font-family:'Pretendard Variable', sans-serif; border:1px solid #d1d5db;">
          <div style="display:flex; flex-direction:column; gap:4px; flex:1;">
    ...
    case 5: // ⚡ 05. CAD 도면
      return `
        <div style="${baseStyle} background:#0a0d14; border:1px solid #1e293b; padding:12px 11px; font-family:'JetBrains Mono', monospace; color:#f8fafc; position:relative;">
          <div style="display:flex; flex-direction:column; gap:4px; flex:1;">
    ...
    case 6: // 📸 06. 코닥 슬라이드
      return `
        <div style="${baseStyle} background:#f5f4ef; color:#18181b; padding:9px 8px 10px 8px; font-family:'Pretendard Variable', sans-serif; border:1px solid #d4d4d8;">
    ...
    case 7: // 📖 07. 매거진
      return `
        <div style="${baseStyle} background:#f4f1ea; color:#1a1918; padding:11px 10px; font-family:'Pretendard Variable', sans-serif;">
          <div style="display:flex; flex-direction:column; gap:4px; flex:1;">
    ...
    case 8: // ☁️ 08. 솜사탕
      return `
        <div style="${baseStyle} background:linear-gradient(180deg, #fff0f5 0%, #f0f9ff 100%); color:#334155; padding:11px 10px; font-family:'Gaegu', cursive;">
          <div style="display:flex; flex-direction:column; gap:4px; flex:1;">
    ...
    case 9: // 🧈 09. 버터
      return `
        <div style="${baseStyle} background:#fffdf5; color:#292524; padding:11px 10px; font-family:'Pretendard Variable', sans-serif;">
          <div style="display:flex; flex-direction:column; gap:4px; flex:1;">
    ...
    case 10: // 🌿 10. 세이지
      return `
        <div style="${baseStyle} background:linear-gradient(180deg, #f0fdf4 0%, #e6f4ea 100%); color:#14532d; padding:11px 10px; font-family:'Playfair Display', serif;">
          <div style="display:flex; flex-direction:column; gap:4px; flex:1;">
    ...
    case 11: // 💖 11. 블러썸
      return `
        <div style="${baseStyle} background:#ffffff; color:#1c1917; padding:11px 10px; font-family:'Pretendard Variable', sans-serif; border:1.2px solid #fecdd3;">
          <div style="display:flex; flex-direction:column; gap:4px; flex:1;">
    ...
    case 12: // 🌸 12. 라벤더
      return `
        <div style="${baseStyle} background:linear-gradient(180deg, #faf5ff 0%, #f3e8ff 100%); color:#581c87; padding:11px 10px; font-family:'Pretendard Variable', sans-serif;">
          <div style="display:flex; flex-direction:column; gap:4px; flex:1;">
    ...
    case 13: // ☁️ 13. 스카이
      return `
        <div style="${baseStyle} background:linear-gradient(180deg, #f0f9ff 0%, #e0f2fe 100%); color:#0c4a6e; padding:11px 10px; font-family:'Space Grotesk', sans-serif;">
          <div style="display:flex; flex-direction:column; gap:4px; flex:1;">
    ...
    case 14: // 🏷️ 14. 다꾸
      return `
        <div style="${baseStyle} background:#faf7f2; color:#292524; padding:11px 10px; font-family:'Gaegu', cursive;">
          <div style="display:flex; flex-direction:column; gap:4px; flex:1;">
    ...
    case 15: // 🍑 15. 살구노을
      return `
        <div style="${baseStyle} background:linear-gradient(180deg, #fff7ed 0%, #ffedd5 100%); color:#431407; padding:11px 10px; font-family:'Pretendard Variable', sans-serif;">
          <div style="display:flex; flex-direction:column; gap:4px; flex:1;">
    ...
    case 16: // 🌙 16. 핑크문
      return `
        <div style="${baseStyle} background:radial-gradient(circle at 80% 20%, #2e0825 0%, #0d020f 70%, #000000 100%); color:#ffffff; padding:11px 10px; font-family:'Cinzel', serif; border:1.2px solid rgba(244,114,182,0.4);">
          <div style="display:flex; flex-direction:column; gap:4px; flex:1;">
    ...
    case 17: // 🍦 17. 민트
      return `
        <div style="${baseStyle} background:linear-gradient(180deg, #ecfdf5 0%, #d1fae5 100%); color:#064e3b; padding:11px 10px; font-family:'Pretendard Variable', sans-serif;">
          <div style="display:flex; flex-direction:column; gap:4px; flex:1;">
    ...
    case 18: // 🍋 18. 레몬
      return `
        <div style="${baseStyle} background:linear-gradient(180deg, #fefce8 0%, #fef9c3 100%); color:#713f12; padding:11px 10px; font-family:'Gaegu', cursive;">
          <div style="display:flex; flex-direction:column; gap:4px; flex:1;">
    ...
    case 19: // ✨ 19. 럭셔리
      return `
        <div style="${baseStyle} background:#121214; color:#f4f4f5; padding:11px 10px; font-family:'Noto Serif KR', serif; border:1px solid #27272a;">
          <div style="display:flex; flex-direction:column; gap:4px; flex:1;">
    ...
    default:
      return `
        <div style="${baseStyle} background:#18181b; padding:11px 10px; border:1px solid #27272a; display:flex; flex-direction:row; gap:6px; font-family:'Noto Serif KR', serif; color:#ffffff;">
